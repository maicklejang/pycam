...
- **휜 문서 평탄화** — 책이나 둥글게 말린 종이처럼 **변이 휘어 있는 페이지**도
  스캐너로 민 것처럼 폅니다.
  1. 네 변을 밝기 경계를 따라 다시 찾고, **꼭짓점을 두 변이 실제로 만나는 곳으로
     옮깁니다**(검출기는 직선 사각형만 맞출 수 있어서, 말린 종이의 꼭짓점은 최대
     60픽셀까지 어긋납니다).
  2. 종이가 카메라에서 멀어지며 눌린 만큼을 되돌립니다. 각 세로선의 길이가 그
     지점의 거리를 알려주므로 종이의 단면을 복원해 **실제 길이 간격으로** 샘플링
     합니다. 평평한 종이에서는 계산이 정확히 상쇄되어 기존 원근 보정과 같습니다.
  3. 남은 왜곡은 **글줄의 휘어짐을 측정해서** 바로잡습니다. 글줄을 따라갈 때는
     **줄 간격의 일부까지만** 이동을 허용하고, 다른 줄들과 어긋나게 휜 줄은
     버립니다. 표의 짧은 칸이나 문단 끝처럼 선이 중간에 끊기는 자리에서 옆 줄로
     옮겨 붙어 **없는 휘어짐을 만들어 내는 것**(글씨가 물결치거나 뭉개지는 증상)을
     막기 위해서입니다.

  합성 사진(원통에 0.9 라디안 감은 종이)으로 잰 남은 왜곡은 가로 1.69 % → **0.41 %**,
  세로 1.40 % → **0.10 %**(종이 너비 대비)입니다. `--no-flatten`으로 끌 수 있습니다.
...
| `docscan.curve.refine_edges(image, quad)` | 실제 종이 경계를 따라 꼭짓점·네 변 다시 찾기 |
...
| `curve.py` | 경계 추적과 꼭짓점 보정(`CurvedQuad`), 평탄화, 글줄 기반 보정 |

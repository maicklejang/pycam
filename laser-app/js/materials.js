/*
 * 소재 데이터베이스 (CO2 레이저 · 파이버 마킹기 전용)
 *
 * 절단 모델 : 필요 선(線)에너지 E[J/mm] = k * 두께^exp * factor[장비종류]
 *             단, 유효 출력이 wPerMm * 두께 보다 낮으면 몇 번을 지나가도 관통되지 않는다
 ...
 * 파이버 마킹기는 주파수·해치가 포함된 fiber 프리셋을 그대로 사용합니다.
 */
...
const MACHINE_PRESETS = [
  ...
  { id: "co2-150", label: "CO2 150W", type: "co2", watt: 150, maxSpeed: 700 },
  { id: "fiber-20", label: "파이버 마킹기 20W", type: "fiber", watt: 20, maxSpeed: 3000 },
  { id: "fiber-30", label: "파이버 마킹기 30W MOPA", type: "fiber", watt: 30, maxSpeed: 4000 },
  { id: "fiber-50", label: "파이버 마킹기 50W MOPA", type: "fiber", watt: 50, maxSpeed: 5000 },
  { id: "fiber-100", label: "파이버 100W (딥 각인)", type: "fiber", watt: 100, maxSpeed: 5000 },
];
...
const MACHINE_TYPE_INFO = {
  co2: {
    ...
    minSpeed: 4,
    maxPasses: 6,
    hint: "튜브 수명을 위해 80% 이상 연속 사용은 피하고, 10% 미만은 방전이 불안정합니다. 출력은 튜브 사양(예: 60W) 기준입니다.",
  },
  fiber: {
    label: "파이버 마킹기 (1064nm)",
    powerCapPct: 100,
    minPowerPct: 10,
    minSpeed: 100,
    maxPasses: 20,
    hint: "금속과 일부 플라스틱 마킹 전용입니다. 목재·아크릴·가죽 절단에는 사용할 수 없습니다.",
  },
};
...
const MATERIALS = [
  /* ---------------- 목재 ---------------- */
  {
    ...
    cut: { k: 0.575, exp: 1.15, wPerMm: 3.5, kerf: 0.15, powerPct: { co2: 70 }, maxThickness: { co2: 20 } },
    engrave: { density: 0.47, dpi: 300, powerPct: { co2: 20 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    cut: { k: 0.78, exp: 1.15, wPerMm: 4.0, kerf: 0.18, powerPct: { co2: 75 }, maxThickness: { co2: 18 } },
    engrave: { density: 0.5, dpi: 300, powerPct: { co2: 20 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    desc: "가볍고 잘 잘리는 모형용 판재. 저출력 장비에서도 여유 있게 잘립니다.",
    thicknesses: [1.5, 2, 3, 4, 5, 6],
    cut: { k: 0.45, exp: 1.15, wPerMm: 3.0, kerf: 0.13, powerPct: { co2: 70 }, maxThickness: { co2: 20 } },
    engrave: { density: 0.42, dpi: 300, powerPct: { co2: 18 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: ["40W급 입문 장비의 첫 연습 소재로 권장합니다."], warns: [],
  },
  {
    id: "hardwood", name: "원목 판재 (오크·월넛·단풍)", category: "wood",
    desc: "밀도가 높아 같은 두께라도 합판보다 느립니다.",
    thicknesses: [3, 5, 8, 10],
    cut: { k: 0.88, exp: 1.15, wPerMm: 4.5, kerf: 0.15, powerPct: { co2: 75 }, maxThickness: { co2: 15 } },
    engrave: { density: 0.55, dpi: 300, powerPct: { co2: 22 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    thicknesses: [3, 5],
    cut: { k: 0.92, exp: 1.15, wPerMm: 4.6, kerf: 0.15, powerPct: { co2: 75 }, maxThickness: { co2: 12 } },
    engrave: { density: 0.6, dpi: 300, powerPct: { co2: 25 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    thicknesses: [2, 3, 5],
    cut: { k: 0.32, exp: 1.1, wPerMm: 2.2, kerf: 0.2, powerPct: { co2: 60 }, maxThickness: { co2: 10 } },
    engrave: { density: 0.4, dpi: 254, powerPct: { co2: 18 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  /* ---------------- 아크릴 ---------------- */
  {
    ...
    cut: { k: 0.62, exp: 1.15, wPerMm: 3.2, kerf: 0.2, powerPct: { co2: 70 }, maxThickness: { co2: 25 } },
    engrave: { density: 0.35, dpi: 300, powerPct: { co2: 18 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    desc: "조각 대비가 강해 명패·간판에 많이 쓰입니다.",
    thicknesses: [2, 3, 5, 8],
    cut: { k: 0.62, exp: 1.15, wPerMm: 3.2, kerf: 0.2, powerPct: { co2: 70 }, maxThickness: { co2: 25 } },
    engrave: { density: 0.35, dpi: 300, powerPct: { co2: 18 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: ["검정 바탕에 흰 글씨를 내려면 조각 출력을 10% 정도 낮추고 속도를 올리세요."], warns: [],
  },
  {
    id: "acrylic-mirror", name: "미러 아크릴", category: "acrylic",
    desc: "뒷면(거울면) 조각으로 반사 문양을 만듭니다.",
    thicknesses: [2, 3],
    cut: { k: 0.66, exp: 1.15, wPerMm: 3.4, kerf: 0.2, powerPct: { co2: 70 }, maxThickness: { co2: 8 } },
    engrave: { density: 0.3, dpi: 300, powerPct: { co2: 15 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: ["도면을 좌우 반전해서 뒷면을 조각하세요."], warns: [],
  },
  ...
  /* ---------------- 종이·가죽·패브릭 ---------------- */
  {
    ...
    cut: { k: 0.12, exp: 1.0, wPerMm: 1.5, kerf: 0.08, powerPct: { co2: 25 }, maxThickness: { co2: 2 } },
    engrave: { density: 0.15, dpi: 254, powerPct: { co2: 10 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    desc: "시제품·포장 목업용.",
    thicknesses: [1, 2, 3],
    cut: { k: 0.18, exp: 1.05, wPerMm: 1.8, kerf: 0.15, powerPct: { co2: 45 }, maxThickness: { co2: 8 } },
    engrave: { density: 0.2, dpi: 254, powerPct: { co2: 12 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    cut: { k: 0.42, exp: 1.1, wPerMm: 2.5, kerf: 0.15, powerPct: { co2: 60 }, maxThickness: { co2: 6 } },
    engrave: { density: 0.4, dpi: 300, powerPct: { co2: 15 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    cut: { k: 0.35, exp: 1.1, wPerMm: 2.2, kerf: 0.15, powerPct: { co2: 50 }, maxThickness: { co2: 4 } },
    engrave: { density: 0.3, dpi: 300, powerPct: { co2: 12 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: [],
  ...
  },
  {
    ...
    thicknesses: [1, 2, 3],
    cut: { k: 0.28, exp: 1.05, wPerMm: 2.0, kerf: 0.2, powerPct: { co2: 45 }, maxThickness: { co2: 5 } },
    engrave: { density: 0.3, dpi: 254, powerPct: { co2: 12 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: [], warns: [],
  },
  {
    ...
    desc: "패치·아플리케용. 저출력 고속으로 태우듯 자릅니다.",
    thicknesses: [0.3, 0.5, 0.8],
    cut: { k: 0.12, exp: 1.0, wPerMm: 1.5, kerf: 0.1, powerPct: { co2: 30 }, maxThickness: { co2: 3 } },
    engrave: { density: 0.2, dpi: 254, powerPct: { co2: 10 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: ["원단이 들뜨면 초점이 어긋납니다. 눌러 고정하세요."], warns: [],
  },
  ...
  /* ---------------- 고무·폼·플라스틱 ---------------- */
  {
    ...
    thicknesses: [2.3, 3],
    cut: { k: 0.55, exp: 1.1, wPerMm: 3.0, kerf: 0.3, powerPct: { co2: 65 }, maxThickness: { co2: 6 } },
    engrave: { density: 2.5, dpi: 200, powerPct: { co2: 60 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    cut: { k: 0.3, exp: 1.1, wPerMm: 2.0, kerf: 0.3, powerPct: { co2: 55 }, maxThickness: { co2: 12 } },
    engrave: { density: 0.4, dpi: 254, powerPct: { co2: 15 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
    tips: [], warns: [],
  },
  {
    ...
    desc: "기계 부품용. 절단면이 깨끗하고 파이버로 마킹도 됩니다.",
    thicknesses: [1, 2, 3, 5],
    cut: { k: 0.95, exp: 1.15, wPerMm: 4.0, kerf: 0.2, powerPct: { co2: 75 }, maxThickness: { co2: 8 } },
    engrave: { density: 0.6, dpi: 300, powerPct: { co2: 25 } },
    factor: { co2: 1, fiber: null },
    engraveFactor: { co2: 1, fiber: 1.0 },
    fiber: { speed: 800, powerPct: 50, freqKhz: 40, passes: 1, hatchMm: 0.03 },
  ...
  /* ---------------- 석재·유리 (조각 전용) ---------------- */
  {
    ...
    thicknesses: [],
    cut: null,
    engrave: { density: 1.2, dpi: 300, powerPct: { co2: 45 } },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    thicknesses: [],
    cut: null,
    engrave: { density: 1.5, dpi: 200, powerPct: { co2: 35 } },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  },
  {
    ...
    desc: "마킹 스프레이를 뿌린 뒤 검게 고착시킵니다.",
    thicknesses: [],
    cut: null,
    engrave: { density: 1.4, dpi: 300, powerPct: { co2: 60 } },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: 1, fiber: null },
  ...
  /* ---------------- 금속 (파이버 마킹기) ---------------- */
  {
    ...
    thicknesses: [],
    cut: null,
    engrave: { density: 0.9, dpi: 400, powerPct: { co2: 70 } },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: 1, fiber: 1.0 },
  ...
  },
  {
    ...
    thicknesses: [],
    cut: null,
    engrave: { density: 4.0, dpi: 800, powerPct: {} },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: null, fiber: 1.0 },
  ...
  {
    id: "titanium", name: "티타늄 (컬러 마킹)", category: "metal",
    desc: "MOPA 파이버에서 주파수·펄스폭을 바꿔 색을 냅니다.",
    thicknesses: [],
    cut: null,
    engrave: { density: 2.2, dpi: 1000, powerPct: {} },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: null, fiber: 1.0 },
    fiber: { speed: 500, powerPct: 30, freqKhz: 200, passes: 1, hatchMm: 0.015 },
    tips: ["색은 주파수(60~800kHz)와 속도 조합으로 결정됩니다. 컬러 팔레트 시험판을 먼저 만들어 두세요.", "일반 파이버(Q-switch)로는 색 마킹이 어렵고 MOPA가 필요합니다."],
    warns: [],
  },
  {
    id: "coated-metal", name: "도장 금속 (텀블러·분체도장)", category: "metal",
    desc: "도장을 벗겨 바탕 금속색을 드러냅니다. 판촉물 각인의 주력 품목입니다.",
    thicknesses: [],
    cut: null,
    engrave: { density: 0.7, dpi: 400, powerPct: { co2: 60 } },
    factor: { co2: null, fiber: null },
    engraveFactor: { co2: 1, fiber: 1.0 },
    fiber: { speed: 2000, powerPct: 30, freqKhz: 30, passes: 1, hatchMm: 0.04 },
    tips: ["출력이 높으면 바탕 금속까지 파여 지저분해집니다. 도장만 벗겨질 만큼만 올리세요.", "원통형은 로터리 축을 쓰고, 없으면 초점 깊이 안쪽 폭(약 20mm)만 마킹하세요."],
    warns: [],
  },
];
